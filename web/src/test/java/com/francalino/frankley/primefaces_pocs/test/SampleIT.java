package com.francalino.frankley.primefaces_pocs.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "primefaces-pocs-ear.ear");

        // Current directory is the root of the "primefaces-pocs-web" project. Go up one level
        // and enter the "ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../ejb/target/primefaces-pocs-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "primefaces-pocs-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/primefaces-pocs-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "primefaces-pocs-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("com.francalino.frankley.primefaces_pocs.test");
